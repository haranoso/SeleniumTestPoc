...
module.exports = class TestUtil{
    ...
    async waitForLoadBy( byData ){
        ...
        await this.driver.wait(until.elementLocated(byData),this.waitTimeout);
        var element = await this.driver.findElement(byData);
        return element;
    }
    ...
    async urlIncludeCheck ( checkStr){
        var url = await this.driver.getCurrentUrl();
        console.log(url);
        let slices = url.replace(/\//g,' ').trim().split(" ").slice(-1)[0];
        console.log(slices.includes(checkStr));
        return slices.includes(checkStr);
    }
}